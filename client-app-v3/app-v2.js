...
async function boot() {
  try {
    ...
    show('#clientApp');
    renderClient();
    const healthResult = new URLSearchParams(location.search).get('health');
    if (healthResult) { toast(healthResult === 'fitbit_connected' ? 'Fitbit connected — sync your latest data now.' : `Fitbit connection: ${healthResult.replaceAll('_',' ')}`, healthResult === 'fitbit_connected' ? '' : 'error'); history.replaceState({}, '', location.pathname); }
  } catch (error) {
...
function clientHealth() {
  ...
  $('#clientMain').innerHTML=clientHeader('ACCOUNT','Connected health','Bring steps, sleep, heart rate, weight and workouts into one private timeline.')+healthSummaryMarkup()+`<section class="panel live-health"><div class="panel-head"><div><span class="eyebrow">LIVE SYNC</span><h3>Fitbit / Pixel Watch</h3><span class="sub">Connect once, then securely refresh the latest steps, sleep, heart rate and weight.</span></div></div><div class="health-sync-actions"><button class="btn primary" id="connectFitbit">Connect Fitbit</button><button class="btn ghost" id="syncFitbit">Sync latest 14 days</button></div><div id="fitbitStatus" class="form-msg"></div></section><section class="panel health-import"><div class="panel-head"><div><span class="eyebrow">APPLE & GOOGLE</span><h3>Import health data</h3><span class="sub">Processed inside your signed-in account. The source file itself is not retained.</span></div></div><form id="healthImportForm" class="health-import-form"><label class="field">Source<select name="provider"><option value="apple_health">Apple Health</option><option value="health_connect">Google Health Connect</option><option value="fitbit">Fitbit / Pixel Watch</option></select></label><label class="field wide">Export file<input name="health_file" type="file" accept=".xml,.csv,.json,text/csv,application/json,text/xml" required></label><button class="btn primary wide">Import and sync</button><div id="healthImportStatus" class="form-msg wide"></div></form><details class="import-help"><summary>How to get the file</summary><p><b>Apple Health:</b> Health app → profile picture → Export All Health Data → upload <code>export.xml</code>.</p><p><b>Google Health Connect:</b> export or download your Health Connect data as CSV/JSON and upload it here.</p><p><b>Fitbit / Pixel Watch:</b> download the health data export from Fitbit/Google Takeout and upload a CSV/JSON file.</p></details></section><section class="panel connection-list"><div class="panel-head"><h3>Connections</h3></div>${providers.map(([id,name,help])=>{const item=(state.data.healthConnections||[]).find(c=>c.provider===id);const connected=item?.status==='connected';return `<article><div><h3>${name}</h3><p>${connected?`Data available · Last synced ${fmt(item.last_synced_at)}`:help}</p></div><span class="pill ${connected?'active':''}">${connected?'CONNECTED':'NOT CONNECTED'}</span></article>`;}).join('')}</section><section class="panel"><div class="panel-head"><div><h3>Import history</h3><span class="sub">An audit trail without retaining the original export.</span></div></div>${imports.length?`<div class="health-import-history">${imports.map(item=>`<div><b>${esc(title(item.provider.replaceAll('_',' ')))}</b><span>${Number(item.imported_rows||0).toLocaleString()} daily records · ${fmt(item.imported_at)}</span><small>${esc(item.file_name||'Health export')} · ${item.date_from?`${fmt(item.date_from)} to ${fmt(item.date_to)}`:'No dated records'}</small></div>`).join('')}</div>`:'<div class="empty">No health files imported yet.</div>'}</section>`;
  $('#healthImportForm').onsubmit=importHealthFile;
  $('#connectFitbit').onclick=connectFitbit;
  $('#syncFitbit').onclick=syncFitbit;
}

async function connectFitbit(event){if(state.preview)return toast('Sign in as a client to connect Fitbit.');setBusy(event.currentTarget,true,'Opening Fitbit…');try{const {data,error}=await db.functions.invoke('fitbit-connect',{body:{client_id:state.client.id}});if(error)throw error;if(data?.error)throw new Error(data.error);location.href=data.authorization_url;}catch(error){$('#fitbitStatus').textContent=error.message;toast(error.message,'error');setBusy(event.currentTarget,false);}}
async function syncFitbit(event){if(state.preview)return toast('Sign in as a client to sync Fitbit.');setBusy(event.currentTarget,true,'Syncing…');try{const {data,error}=await db.functions.invoke('fitbit-sync',{body:{client_id:state.client.id,days:14}});if(error)throw error;if(data?.error)throw new Error(data.error);toast(`${data.synced} Fitbit days synced`);await loadClientData(state.client.id);clientHealth();}catch(error){$('#fitbitStatus').textContent=error.message;toast(error.message,'error');setBusy(event.currentTarget,false);}}
